...
static int huff_validate_file_name(char *f_name, int f_type)
{
    ...
    int min_fnlen = (f_type == HUFFMAN_OPT_ENCODE) ? 1 :
	1 + HUFFMAN_SUFFIX_LENGTH;
...
#define BUFFER_LENGTH 10
    ...
    else if (file_length < KILO)
	snprintf(length_buf, BUFFER_LENGTH - 1, "%lubytes", file_length);
    else if (file_length < MEGA)
    {
	snprintf(length_buf, BUFFER_LENGTH - 1, "%.2fKb",
	    (double)file_length/KILO_BYTE);
    }
    else if (file_length < GIGA)
    {
	snprintf(length_buf, BUFFER_LENGTH - 1, "%.2fMb",
	    (double)file_length/MEGA_BYTE);
    }
    else
    {
	snprintf(length_buf, BUFFER_LENGTH - 1, "%.2fGb",
...
static void huff_print_verbose(void)
{
    ...
    /* computing character representation length mean, varience and stdandard
     * deviation.
    ...
    /* computing max_rep_length, min_rep_length, varince and breakdown table */
    memset(breakdown, 0, ANSI_CHAR_SET_CARDINALITY);
    for (i = 0; i < ANSI_CHAR_SET_CARDINALITY; i++)
    {
	if (frequency[i])
	{
	    min_rep_length = MIN(min_rep_length, representation_length[i]);
	    max_rep_length = MAX(max_rep_length, representation_length[i]);
	    var += frequency[i] * pow(representation_length[i] - mean, 2);
    ...
    printf("\ncharacter representation length (bits)     number of characters"
	"\n");
    printf("--------------------------------------     --------------------\n");
